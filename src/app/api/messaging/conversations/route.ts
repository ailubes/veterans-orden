...
import { canInitiateDMs, canCreateGroupChats, canMessageUser } from '@/lib/messaging/permissions';
...
export async function POST(request: Request) {
  try {
    ...
    const { data: profile } = await supabase
      .from('users')
      .select('id, membership_role, referred_by_id')
      .eq('clerk_id', user.id)
      .single();
    ...
    if (type === 'direct') {
      ...
      const canMessage = canMessageUser({
        ...
        isDirectReferral,
        isInLeaderGroup,
        isSameGroup: false, // TODO: implement same group check
        settings,
      });
      ...
      if (!canCreateGroupChats(profile.membership_role, settings)) {
        return NextResponse.json(
