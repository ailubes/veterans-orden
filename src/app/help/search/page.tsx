  ...
  if (!query.trim()) {
    return (
      <div className="bg-white border border-line rounded-lg p-12 text-center relative">
        ...
        <Search className="mx-auto mb-4 text-muted-500" size={64} />
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-4 text-muted-500">Пошук...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-white border border-line rounded-lg p-12 text-center relative">
        ...
        <p className="text-muted-500">{error}</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white border border-line rounded-lg p-8 relative">
        ...
        <div className="flex items-center gap-3 mb-4">
          <Search className="text-bronze" size={32} />
          <h1 className="font-syne text-3xl font-bold">Результати пошуку</h1>
        </div>

        <p className="text-lg mb-2">
          Знайдено <span className="font-bold text-bronze">{results.length}</span>{' '}
      ...
      {results.length > 0 ? (
            ...
            >
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-shrink-0">
                  <div className="w-12 h-12 bg-bronze/10 border-2 border-bronze flex items-center justify-center">
                    <span className="font-syne font-bold text-bronze">#{index + 1}</span>
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h2 className="font-syne text-xl font-bold mb-3 group-hover:text-bronze transition-colors">
                    {highlightText(result.title, query)}
                  </h2>

                  {/* Excerpt with highlighting */}
                  {result.excerpt && (
                    <p className="text-sm text-muted-500 line-clamp-3">
                      {highlightText(result.excerpt, query)}
                    </p>
                  )}

                  {/* Match indicator */}
                  <div className="flex items-center gap-2 mt-3 text-xs text-muted-500">
                    <Sparkles size={14} className="text-bronze" />
                    <span>Відповідає вашому запиту</span>
                  </div>
                </div>

                <FileText className="text-muted-500 group-hover:text-bronze transition-colors flex-shrink-0" size={24} />
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="bg-white border border-line rounded-lg p-12 text-center relative">
          ...
          <Link
            href="/help"
            className="inline-block mt-6 px-6 py-3 bg-bronze text-canvas font-bold hover:shadow-[4px_4px_0px_0px_rgba(44,40,36,1)] transition-all"
          >
