...
import { Activity, Phone, Globe, MapPin, ChevronLeft, CheckCircle, XCircle } from 'lucide-react';
...
export default function HealthcareResourcesPage() {
  ...
  useEffect(() => {
    fetch('/api/resources/healthcare')
      .then((r) => r.json())
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="mono text-bronze text-xs tracking-widest mb-2">// ОХОРОНА ЗДОРОВ'Я</p>
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-lg bg-green-500/10 border border-green-500/30 flex items-center justify-center">
            <Activity size={20} className="text-green-400" />
          </div>
          <h1 className="font-syne text-3xl font-bold text-text-100">Охорона здоров&apos;я</h1>
        </div>
        <p className="text-muted-500 text-sm">
          Реабілітаційні центри, програми протезування, санаторне лікування та медичні програми для ветеранів.
        </p>
      </div>
...
function ResourceCard({ resource }: { resource: Resource }) {
  ...
  return (
    <div className="bg-panel-900 border border-line rounded-lg p-5 hover:border-green-500/30 transition-colors">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {resource.contact && (
          isUrl ? (
            ...
            <a href={`tel:${resource.contact}`} className="flex items-center gap-1 text-xs text-bronze hover:underline">
