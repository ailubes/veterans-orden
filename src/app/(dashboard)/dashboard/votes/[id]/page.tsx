  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <Link
        ...
        className="text-sm text-muted-500 hover:text-bronze flex items-center gap-2 mb-6"
      >
        <ArrowLeft size={16} />
        Назад до голосувань
      </Link>
      ...
      {/* Vote Header */}
      <div className="bg-panel-900 border border-line p-6 rounded-lg mb-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          {isActive ? (
            ...
            <span className="px-2 py-1 bg-panel-850 text-muted-500 text-xs font-bold rounded">
              ЗАВЕРШЕНО
            </span>
          )}
        </div>
        ...
        {vote.description && (
          <p className="text-muted-500 mb-6">{vote.description}</p>
        )}

        <div className="flex flex-wrap gap-6 text-sm text-muted-500">
      ...
      <div className="bg-panel-900 border border-line p-6 rounded-lg">
        ...
        {/* Already voted */}
        {hasVoted && (
          <div className="bg-green-500/10 border border-green-500/30 p-4 mb-6 text-sm text-green-400 rounded">
        ...
        {(hasVoted || !isActive) && (
          <div className="space-y-4">
            ...
            {optionsWithPercentage.map((option) => (
              <div key={option.id} className="space-y-2">
                <div className="flex items-center justify-between">
                  <span
                    className={`font-bold ${
                      userVote === option.id ? 'text-bronze' : 'text-text-100'
                    }`}
                  >
                    {option.text}
                    {userVote === option.id && ' (ваш голос)'}
                  </span>
                  <span className="font-mono text-sm text-muted-500">
                    {option.vote_count || 0} ({option.percentage.toFixed(1)}%)
                  </span>
                </div>
                <div className="h-3 bg-panel-850 rounded-full relative overflow-hidden">
                  <div
                    className={`absolute left-0 top-0 bottom-0 transition-all rounded-full ${
                      userVote === option.id ? 'bg-bronze' : 'bg-text-200'
                    }`}
            ...
            <div className="pt-4 border-t border-line text-sm text-muted-500">
