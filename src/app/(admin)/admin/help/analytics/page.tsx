...
export default function HelpAnalyticsPage() {
  ...
  return (
    <div className="p-8 bg-panel-900 min-h-screen">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-8">
          ...
          <select
            ...
            className="px-4 py-2 border border-line rounded-lg bg-white focus:outline-none focus:border-bronze"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-white border border-line rounded-lg p-6 relative">
            ...
            <FileText className="text-muted-500 mb-2" size={24} />
            <p className="text-sm text-muted-500 uppercase tracking-wider mb-1">
              Всього статей
            </p>
          ...
          <div className="bg-white border border-line rounded-lg p-6 relative">
            ...
            <MessageSquare className="text-muted-500 mb-2" size={24} />
            <p className="text-sm text-muted-500 uppercase tracking-wider mb-1">
              Відгуків
            </p>
        ...
        {/* Top Viewed Articles */}
        <div className="bg-white border border-line rounded-lg p-6 mb-8 relative">
          <div className="joint joint-tl" />
          <div className="joint joint-tr" />
          <div className="joint joint-bl" />
          <div className="joint joint-br" />
          ...
          <div className="flex items-center gap-2 mb-4">
            <BarChart3 className="text-bronze" size={24} />
            ...
            <table className="w-full">
              ...
              <tbody>
                {data.topViewedArticles.map((article, index) => (
                  <tr key={article.id} className="border-b border-line/20">
                    <td className="py-3 px-2">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-muted-500">#{index + 1}</span>
                        <span>{article.title}</span>
                      </div>
                    </td>
                    <td className="py-3 px-2 text-sm text-muted-500">
                    ...
                    <td className="py-3 px-2 text-right">
                      {article.helpfulRate !== null ? (
                        ...
                        <span className="text-muted-500/50">—</span>
                      )}
                    </td>
                    <td className="py-3 px-2 text-right">
                      <Link
                        href={`/help/${article.slug}`}
                        className="text-bronze hover:underline text-sm"
                        target="_blank"
                      >
                        Переглянути
                      </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white border border-line rounded-lg p-6 relative">
            ...
            {data.lowPerformingArticles.length === 0 ? (
              <p className="text-muted-500/70 text-sm">
              ...
              <div className="space-y-3">
                {data.lowPerformingArticles.map((article) => (
                  <div
                    key={article.id}
                    className="p-3 border border-line/20 flex items-center justify-between"
                  >
          ...
          <div className="bg-white border border-line rounded-lg p-6 relative">
            ...
            <div className="space-y-3">
              {data.categoryPerformance.map((category) => (
                <div
                  key={category.id}
                  className="p-3 border border-line/20"
                >
        ...
        <div className="bg-white border border-line rounded-lg p-6 relative">
          ...
          {data.recentFeedback.length === 0 ? (
            <p className="text-muted-500/70 text-sm">Поки що немає коментарів</p>
          ) : (
            <div className="space-y-4">
              {data.recentFeedback.map((feedback) => (
                <div
                  key={feedback.id}
                  className="p-4 border border-line/20"
                >
