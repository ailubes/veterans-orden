...
CREATE INDEX "help_categories_parent_idx" ON "help_categories" ("parent_id");
...
CREATE INDEX "help_articles_author_idx" ON "help_articles" ("author_id");
CREATE INDEX "help_articles_audience_idx" ON "help_articles" ("audience");

-- Create indexes for help_article_feedback
CREATE INDEX "help_article_feedback_article_idx" ON "help_article_feedback" ("article_id");
CREATE INDEX "help_article_feedback_user_idx" ON "help_article_feedback" ("user_id");

-- Create indexes for help_tooltips
CREATE UNIQUE INDEX "help_tooltips_page_element_idx" ON "help_tooltips" ("page_slug", "element_id");
CREATE INDEX "help_tooltips_article_idx" ON "help_tooltips" ("article_id");

-- Create trigger to auto-update search_vector for Ukrainian full-text search
CREATE OR REPLACE FUNCTION help_articles_search_vector_update()
RETURNS trigger AS $$
BEGIN
  NEW.search_vector :=
    setweight(to_tsvector('pg_catalog.ukrainian', coalesce(NEW.title, '')), 'A') ||
    setweight(to_tsvector('pg_catalog.ukrainian', coalesce(NEW.content, '')), 'B') ||
    setweight(to_tsvector('pg_catalog.ukrainian', coalesce(NEW.excerpt, '')), 'C');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Create function for searching help articles
...
BEGIN
  ...
  SELECT
    ...
    c.name_uk,
    (
      -- Title exact match (rank 1.0)
      CASE WHEN a.title ILIKE search_query THEN 1.0
      -- Title similarity > 0.3 using pg_trgm (rank 0.8)
      WHEN similarity(a.title, search_query) > 0.3
        THEN similarity(a.title, search_query) * 0.8
      -- Full-text search on content (rank 0.6)
      WHEN a.search_vector @@ plainto_tsquery('pg_catalog.ukrainian', search_query)
        THEN ts_rank(a.search_vector, plainto_tsquery('pg_catalog.ukrainian', search_query)) * 0.6
      -- Keyword match (rank 0.4)
      WHEN a.keywords::text ILIKE '%' || search_query || '%' THEN 0.4
      ELSE 0.0
      END
    )::real AS rank
  ...
  WHERE a.status = 'published'
    AND (
      ...
      OR a.search_vector @@ plainto_tsquery('pg_catalog.ukrainian', search_query)
